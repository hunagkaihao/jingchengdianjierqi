...
namespace Wms.PlcTool;
    ...
    {
        private readonly IRedisClient _WmsRedisClient;
            ...
            IRedisClient WmsRedisClient)
        {
            try
            {
                ...
                _WmsRedisClient = WmsRedisClient;
                _plcRedisClient.Build(_options.Value.RedisConnStr, _options.Value.PlcRedisNo);
                _WmsRedisClient.Build(_options.Value.RedisConnStr, _options.Value.DefaultRedisNo);
                ...
                {
                    string[] fields = _WmsRedisClient.GetHashFields(TagTempChannelName);
                    _WmsRedisClient.RemoveHashFields(TagTempChannelName, fields);
                }
            }
            catch (Exception ex)
            {
        ...
        private string GetPlcTagTemp(string plcName, string tagName)
        {
            try
            {
                return _WmsRedisClient.GetHashValue(TagTempChannelName, $"{plcName}.{tagName}");
            }
            catch (Exception e)
            {
                _logger.Error(e.Message);
        ...
        private void SetPlcTagTemp(string plcName, string tagName, string value)
        {
            try
            {
                _WmsRedisClient.SetHashValue(TagTempChannelName, $"{plcName}.{tagName}", value);
            }
            catch (Exception e)
            {
                _logger.Error(e.Message);
