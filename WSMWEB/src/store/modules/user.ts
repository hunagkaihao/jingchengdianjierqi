...
export const useUserStore = defineStore({
  ...
  getters: {
    ...
    getStationAccount():string{
      return this.stationAccount || getAuthCache<string>(ABP_TETANT_KEY); ;
    },
  ...
  actions: {
    ...
    setStationAccount(station:string){
      this.stationAccount = station;
      setAuthCache(ABP_TETANT_KEY, station);
    },
    resetState() {
      this.userInfo = null;
      this.roleList = [];
    ...
    async mobilelogin(
      ...
      try {
        ...
        await this.getAbpApplicationConfigurationAsync();
        goHome && (await router.replace(PageEnum.BASE_MOBILEHOME));
        return null;
      } catch (error) {
        console.log(error);
        router.replace(PageEnum.BASE_MOBILELOGIN);
        return null;
      }
    },
    ...
    async getAbpApplicationConfigurationAsync() {
      ...
      console.log(grantPolicy.length)
    ...
    async mobilelogout(goLogin = false) {
      try {
        if (this.userInfo?.isSts) {
          await useOidcLogout();
        } else {
          this.resetState();
          goLogin && router.push(PageEnum.BASE_MOBILELOGIN);
        }
      } catch (ex) {
        console.log(ex);
      }
