<template>
    <div class="components-input-demo-presuffix">
        <Header numb="托盘组盘入库"></Header>
        <a-row style="margin-top: 10px;">
            <a-col :span="6">
                <div class="htext">
                    <h1 autofocus="autofocus">绑定托盘:</h1>
                </div>
            </a-col>
            <a-col :span="17">
        ...
        <a-row style="margin-top: 10px;">
            ...
            <a-col :span="17">
                <a-input v-model:value="cellCode" placeholder="扫描入库库位" @keyup.enter="scancellCode" :allowClear="true"
                    @focus="focusFn">
                    <template #suffix>
                        <scan-outlined />
                    </template>
...
const createLiftIn = async () => {
  message.info('功能已禁用');
  return null;
};
import { StockCreateDto,CellDto } from '/@/services/ServiceProxies';
...
let baoshu = ref<string>('');
let sanjianshu = ref<string>('');
...
async function scangoodsCode() {
    ...
    if(endcellCode.value == ''){
        await getByStock(goodsCode.value,boxCode.value).then((res)=>{
...
const incell = async () => {
    ...
    try{
        await createLiftIn(boxCode.value,cellCode.value,endcellCode.value,stockCreateDto).then((res) => {
            if (res.success == true) {
                //创建入库任务
